...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        if (!message.member.voice.channel) return client.sendTime(message.channel, "❌ | **Bir şeyler çalmak için bir ses kanalında bulunmalısınız!**");
        if (message.guild.me.voice.channel && message.member.voice.channel.id !== message.guild.me.voice.channel.id) return client.sendTime(message.channel, ":x: | **Bu komutu kullanmak için benimle aynı ses kanalında olmalısın!**");
        let SearchString = args.join(" ");
        if (!SearchString) return client.sendTime(message.channel, `**kullanım - **\`${GuildDB.prefix}play [Şarkı ismi | Şarkı URL]\``);
        let CheckNode = client.Manager.nodes.get(client.botconfig.Lavalink.id);
        let Searching = await message.channel.send("<a:YklenmeGif:844222221614317658> Aranıyor...");
        if (!CheckNode || !CheckNode.connected) {
       return client.sendTime(message.channel,"❌ | **Lavalink şuanda bağlanılamıyor...**");
        }
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setColor("GREEN")
                    SongAddedEmbed.setAuthor(`Spotify Playlisti sıraya eklendi`, "https://media.giphy.com/media/r1vgXWBC6FWM5mieCt/giphy.gif");
                    SongAddedEmbed.addField("Sıralanan", `\`${Searched.tracks.length}\` listelendi`, false);
                    ...
                    SongAddedEmbed.setAuthor(`Plağı ekliyorum dostum!!`, "https://media.giphy.com/media/PNpqJ0Oge0hR2VBAy9/giphy.gif");SongAddedEmbed.setColor("RED");
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**Eşleşme bulunamadı - **" + SearchString);
                    ...
                    SongAddedEmbed.addField("Sıralanan", `\`${Searched.tracks.length}\` şarkı`, false);
                    SongAddedEmbed.addField("Playlist Süresi", `\`${prettyMilliseconds(Searched.playlist.duration, { colonNotation: true })}\``, false);
                    Searching.edit(SongAddedEmbed);
                } else {
                    player.queue.add(Searched.tracks[0]);
                    if (!player.playing && !player.paused && !player.queue.size) player.play();
                    SongAddedEmbed.setAuthor(`Plağı ekliyorum dostum!!`, "https://media.giphy.com/media/PNpqJ0Oge0hR2VBAy9/giphy.gif");
                    SongAddedEmbed.setColor("RED");
                    SongAddedEmbed.setThumbnail(Searched.tracks[0].displayThumbnail());
